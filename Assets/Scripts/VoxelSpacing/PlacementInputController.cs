...
public class PlacementInputController : MonoBehaviour, IUIActions
{
    ...
    private PlaceableObject RaycastForPlaceable()
    {
        ...
        if (!Physics.Raycast(ray, out RaycastHit hit, Mathf.Infinity, _placementRaycastMask, QueryTriggerInteraction.Ignore)) return null;
    ...
    private void TryPickUpToInventory()
    {
        ...
        Ray ray = _mainCamera.ScreenPointToRay(_pointerPos);
        if (!Physics.Raycast(ray, out RaycastHit hit, Mathf.Infinity, _placementRaycastMask, QueryTriggerInteraction.Ignore)) return;
