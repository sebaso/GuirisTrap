...
public class Table : MonoBehaviour
{
    ...
    private int _fallbackCapacity = 2; // Default capacity if no chairs are detected
    public int Capacity
    {
        get
        {
            if (_registeredChairs.Count > 0) return _registeredChairs.Count;
            if (_autoGeneratedSeats.Count > 0) return _autoGeneratedSeats.Count;

            // Last resort: use a fallback capacity so tables can still function
            // even when chairs aren't detected on the correct layer
    ...
    public List<Transform> GetSeatPoints()
    {
        ...
        // Auto-generate seat points as fallback when no chairs are registered
